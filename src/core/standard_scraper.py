...
from config import SEARCH_TERMS, HEADER_SETS, SCRAPING
from utils import Logger, HighPerformanceTimer, retry_with_backoff
...
class StandardEbayScraper(BaseScraper):
    ...
    def scrape_all_terms(self) -> List[dict]:
        ...
        Logger.success(f"Scraping complete! Found {len(final_results)} unique GBA auctions")
        if removed > 0:
