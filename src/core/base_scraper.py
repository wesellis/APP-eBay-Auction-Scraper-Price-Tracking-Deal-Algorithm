...
from config import SELECTOR_CHAINS, GBA_KEYWORDS, IMAGE_UPGRADES, DEFAULT_IMAGE, SCRAPING
from utils import Logger, clean_title, is_gba_related, format_price, calculate_similarity
...
class BaseScraper:
    ...
    def extract_listing_data(self, listing_soup) -> Optional[ListingData]:
        ...
        try:
            # Extract title
            title_elem = self.find_element_with_selectors(listing_soup, SELECTOR_CHAINS['title'])
            if not title_elem:
                return None
            ...
            price_elem = self.find_element_with_selectors(listing_soup, SELECTOR_CHAINS['price'])
            price = format_price(price_elem.get_text(strip=True) if price_elem else "")
            
            link_elem = self.find_element_with_selectors(listing_soup, SELECTOR_CHAINS['link'])
            link = link_elem.get('href', '') if link_elem else ''
            
            img_elem = self.find_element_with_selectors(listing_soup, SELECTOR_CHAINS['image'])
            img_src = ''
            if img_elem:
            ...
            time_elem = self.find_element_with_selectors(listing_soup, SELECTOR_CHAINS['time_left'])
